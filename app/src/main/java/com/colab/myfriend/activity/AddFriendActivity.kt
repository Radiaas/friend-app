...
class AddFriendActivity : AppCompatActivity() {
    ...
    private fun saveFriendData() {
        ...
        lifecycleScope.launch {
            if (oldFriend == null) {
                viewModel.insertFriend(Friend(name, school, bio, photoFile.absolutePath))
            } else {
                viewModel.editFriend(
                    oldFriend!!.copy(name = name, school = school, bio = bio, photoPath = photoFile.absolutePath).apply {
                        id = idFriend
                    }
                )
            }
